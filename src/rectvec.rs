...
impl<T: Clone + PartialEq> RectVec<T> {
    /// Creates a new `RectVec`
...
impl<T> From<Vec<Vec<T>>> for RectVec<T> {
    /// Converts `Vec<Vec<T>>` to `RectVec<T>`
...
impl<'a, T> IntoIterator for &'a RectVec<T> {
    type Item = &'a T;
...
impl<'a, T> IntoIterator for &'a mut RectVec<T> {
    type Item = &'a mut T;
...
impl<T: fmt::Display> fmt::Display for RectVec<T> {
